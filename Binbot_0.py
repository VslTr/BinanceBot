...
while True:
    ...
    def func_sell():
        ...
        t_history = client.get_my_trades(symbol=TICKER)  # trade_history (по умолчанию 500 последних ордеров)
        if t_history[-1]["isBuyer"] == True:  # [-1] последний, т.к первым 0-вым берется самый старый ордер(первый)
            buy_price = t_history[-1]['price']  # Запросили по какой ставке были покупки
            buy_volume = t_history[-1]['qty']  # запросили сколько купили
            volume1 = volume1 + float(buy_volume) * float(buy_price)
            # считаем объем монет (btc, usd) потраченных на покупку
        ...
        elif t_history[-1]['isBuyer'] == False:
        ...
        balance = client.get_asset_balance(asset=coin2)['free']
        ...
        set_sell_order = client.order_limit_sell(symbol=TICKER, quantity=round((float(balance)), mq),
    ...
    th = client.get_my_trades(symbol=TICKER)  # trade_history (по умолчанию 500 последних ордеров)
    th_len = len(th)
    ...
    if th_len > 0:  # P1 проверка 1
        print('P1 start')
        if th[-1]["isBuyer"] == True:  # [-1] последний, т.к первым 0-вым берется самый старый ордер(первый)
            print('P2 start')
            ...
            print('P2 stop')
            if float(cash_coin2) >= coin2_minQty:  # P3
                print('P3 start')
                ...
                func_buy2()
                print('P4 stop')
        elif th[-1]['isBuyer'] == False and open_orders_len > 0:  # P5
            print('P5 start')
            ...
            func_bids()
            print('P5 stop')
        elif th[-1]['isBuyer'] == False and open_orders_len == 0:  # P6
            print('P6 start')
            print(colored('ОРДЕР SELL ИСПОЛНЕН', 'green', attrs=['bold']))
            print('')
            print(
